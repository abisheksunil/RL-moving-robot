...
class GazeboRuntime(runtime.Runtime):
    ...
    def __init__(
        ...
        world: str = "/home/abi/Documents/Movingrobot_flat_sdf/world.sdf",
        **kwargs,
    ):
    ...
    def step(self, action: Action) -> State:
        ...
        # Step the simulator
        # print("Changing to realtime here")
        for x in range(1000):
            self.gazebo.run(paused=False)
    ...
    def gazebo(self) -> scenario.GazeboSimulator:
        ...
        gazebo = scenario.GazeboSimulator(
            1.0 /
            self._physics_rate, self._real_time_factor, int(
                num_of_steps_per_run)
        )
    ...
    def world(self) -> scenario.World:
        ...
        if self._gazebo.initialized():
            raise RuntimeError(
                "Gazebo was already initialized, cannot insert world")
        ...
        # Load the world
        ok_world = self._gazebo.insert_world_from_sdf(
            self._world_sdf, self._world_name)
